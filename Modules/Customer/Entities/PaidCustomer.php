...
class PaidCustomer extends BaseModel
{
    ...
    *******************************************/
    ...
    {
        $query = DB::table('customers as c')
                ->selectRaw('c.*,cg.group_name, ((select ifnull(sum(debit),0) from transactions where chart_of_account_id= b.id)-(select ifnull(sum(credit),0) from transactions where chart_of_account_id= b.id)) as balance')
                ->leftjoin('chart_of_accounts as b', 'c.id', '=', 'b.customer_id')
                ->leftjoin('customer_groups as cg', 'c.customer_group_id', '=', 'cg.id')
                ->groupBy('c.id','b.id')
                ->having('balance','<=',0);
